...
export const typeLabels = {
  ...
  unclear: "Unclear answers (OpenAI guidance)",
};
const unclearSourcePattern =
  /WARNING - community\/source answer is not fully clear/i;
export function hasUnclearSourceAnswer(q) {
  const source = q?.sourceAnswer || "";
  return (
    unclearSourcePattern.test(source) ||
    /No clear answer was captured/i.test(source)
  );
}
export function isOpenAIOnlyAnswer(q) {
  return hasUnclearSourceAnswer(q) && Boolean(q?.reviewAnswer);
}
...
export function selectQuestions(config, random = Math.random) {
  ...
  if (config.mode === "random")
    ids = shuffled(ids, random).slice(0, Number(config.size));
  else if (config.shuffle) ids = shuffled(ids, random);
  return ids;
}
...
export function grade(q, answer, basis = "reviewed") {
  ...
  return {
    status:
      earned === possible ? "correct" : earned > 0 ? "partial" : "incorrect",
    earned,
    possible,
  };
}
...
export function elapsed(session, now = Date.now()) {
  return (
    session.elapsed +
    (session.runningSince ? Math.max(0, now - session.runningSince) : 0)
  );
}
